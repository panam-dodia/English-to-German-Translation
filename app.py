...
def load_model():
    ...
    try:
        ...
        checkpoint = torch.load('best_model_en_de.pth', map_location=torch.device('cpu'))
...
def main():
    ...
    with col1:
        ...
        input_text = st.text_area(
            ...
            key="input_text_area"  # Added unique key
        )
        ...
        for col, (name, text) in zip(cols, examples.items()):
            if col.button(name, key=f"btn_{name.lower()}"):  # Added unique keys for buttons
                input_text = text
    ...
    with col2:
        ...
        translation_area = translation_placeholder.text_area(
            ...
            height=200,
            disabled=True,
            key="output_text_area"  # Added unique key
        )
    
    # Add translation button
    if st.button("Translate", type="primary", key="translate_button"):  # Added unique key
        if input_text:
            try:
                with st.spinner("Translating..."):
                    ...
                    translation_placeholder.text_area(
                        ...
                        height=200,
                        disabled=True,
                        key="translation_result"  # Added unique key
                    )
                    
                    # Add copy button
                    if st.button("📋 Copy Translation", key="copy_button"):  # Added unique key
                        st.code(translation)
            except Exception as e:
